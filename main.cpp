...
void drawPegs()//Sticks
{
	int i;
	glColor3f(0.5,0.0,0.1);
	//glColor3f(0.5,0.0,1.1);//coloring the sticks
	for(i=0;i<3;i++)
	{
		glPushMatrix();
...
void display()//Disks Creation
{
	...
	for(i=0;i<3;i++)
	{
		...
		for(j=0;j<=top[i];j++)
		{
			...
			glRotatef(90,1,0,0);
			glColor3f(0.1*POLES[i][j],0.2*POLES[i][j],0);//To Change Color of Disks
...
void first()
{
	...
	glColor3f(1,0,0);
	strokeString(50,850,0.15,0.15,"CANARA ENGINEERING COLLEGE",2);
	...
	glColor3f(1,1,0);
	strokeString(650,160,0.18,0.18,"SHREEJESH T",2);
	//strokeString(650,30,0.2,0.2,"SUKHMA SHETTY(Associate Professor)",2);
	strokeString(650,60,0.18,0.18,"SATHVIK",2);
	glColor3f(1,0,0);
	strokeString(650,120,0.18,0.18,"4CB20CS101",2);
	strokeString(650,30,0.18,0.18,"4CB20CS091",2);
